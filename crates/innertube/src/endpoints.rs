...
impl InnerTube {
    ...
    ///
    /// `params` replays a mood chip: the chip's token narrows the same radio to that mood
    /// (None = the unfiltered mix). Opaque — passed through, never interpreted.
    pub async fn next(
        &self,
        metadata_client: &YouTubeClient,
        video_id: Option<&str>,
...
/// `edit_playlist` answers a 200 even when it refused the edit, so the status field is the signal:
/// `STATUS_FAILED` with no `dedupeOption` is a refusal; with one, it's the duplicate-add guard.
