<script lang="ts">
	...
	import { enqueue, openAddManyToPlaylist } from '$lib/player.svelte';
	...
	// Fork note: upstream also gates the add button on `ui.addPending` (its picker-busy flag);
	// this fork has no such flag, so in-flight queue/play calls (`busy`), an in-flight select-all
	// walk, and unresolved selected pages are the whole gate.
	...
	async function queue(next: boolean) {
		...
		try {
			...
			// Fork backend: `enqueue` fans out to the existing playNext/addToQueue invoke calls.
	...
	>
		<div class="flex flex-wrap items-center justify-center gap-1" role="group" aria-label={t('selection.actions')}>
			...
			{#if selection.count}
				...
				{#if onMove}
					<Button variant="ghost" size="icon" disabled={blocked} onkeydown={onKey}
						title={t('selection.move')} aria-label={t('selection.move')}
						onclick={onMove}>
						<HugeiconsIcon icon={MoveToIcon} class="h-4 w-4" />
					</Button>
				{/if}
				{#if onRemove}
					<Button variant="ghost" size="icon" disabled={blocked} onkeydown={onKey}
