...
impl PlayerJsFetcher {
    ...
    /// Delete cached `player_*.js` + `current_hash.txt` + any legacy discovery verdicts (NOT the
    /// shared config files). context/05. Forces the next `fetch` to re-download and the next
