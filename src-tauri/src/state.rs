...
pub struct AppState {
    ...
    /// Real-time spectrum (WASAPI app-loopback + FFT). Spawn-once thread, toggled by setting.
    pub visualizer: Arc<crate::visualizer::Visualizer>,
    ...
    /// Previous track's videoId, for per-pair crossfade overrides (applied in `start_current`
    /// before it becomes the current one again below).
    last_started: std::sync::Mutex<Option<String>>,
...
impl AppState {
    ...
    pub fn new(
        ...
        AppState {
            ...
            visualizer: Arc::new(crate::visualizer::Visualizer::new()),
        }
    }
    ...
    pub fn disabled_clients(&self) -> HashSet<String> {
    ...
    /// Validates SAPISID presence, sets the cookie on the transport, and resolves which channel(s)
    /// this account can act as. One channel stays seamless; a brand-new multi-channel login pauses
    /// and asks the UI to choose before finalizing.
    ///
    ...
    pub async fn sign_in(&self, cookie: String) -> Result<SignInOutcome, String> {
        ...
        let chosen = persisted_id.as_deref().and_then(|id| {
            identities
                .iter()
                .find(|identity| identity.data_sync_id == id)
        });
        if let Some(identity) = chosen {
            ...
            self.activate_identity(identity, identities.len() > 1, client)
                .await
                .inspect_err(|_| {
                    self.restore_auth_transport(
                        previous_cookie.clone(),
        ...
        if identities.len() > 1 {
            ...
            self.it.set_data_sync_id(None);
            self.db
                .set_pending_auth_selection(&cookie)
                .map_err(|error| {
                    self.restore_auth_transport(
                        previous_cookie.clone(),
                        previous_data_sync_id.clone(),
    ...
    async fn activate_identity(
        ...
        let refreshed = match self
            .it
            .account_menu_for_identity(client, &identity.data_sync_id)
            .await
        {
            Ok(info) if info.name.is_some() => info,
            Ok(_) => {
                return Err("YouTube did not confirm that channel. Try signing in again.".into());
            }
            Err(error) => {
    ...
    async fn start_current(self: &std::sync::Arc<Self>, gen: u64) -> bool {
        ...
        // Per-pair crossfade override (`fromId__toId` → secs), else the global duration.
        // Gapless auto-advances don't pass through here (mpv advances internally), so those
        // always use the global — documented on the queue menu item.
        {
            let prev = self
                .last_started
                .lock()
                .unwrap()
                .replace(item.video_id.clone());
            let secs = prev
                .as_deref()
                .and_then(|p| self.crossfade_override(p, &item.video_id));
            let cf = self.player.get_crossfade();
            let _ = self.player.set_crossfade(secs.unwrap_or(cf.secs), &cf.mode);
        }
    ...
    /// Per-pair crossfade override (`fromId__toId` → secs), if the queue menu set one.
    /// Reads the settings blob fresh per transition — pairs change rarely, tracks change often.
    fn crossfade_override(&self, from: &str, to: &str) -> Option<f64> {
        let raw = self.db.get_setting("crossfade_overrides")?;
        let map: std::collections::HashMap<String, f64> = serde_json::from_str(&raw).ok()?;
        map.get(&format!("{from}__{to}")).copied()
    }
...
mod tests {
    ...
    fn sleep_mode_parses_all_shapes() {
        ...
        assert_eq!(
            parse_sleep_mode("end_of_song").unwrap(),
